...
--,UnionChurners AS (
SELECT f.*,voluntario, involuntario, tenure AS TenureChurners
...
,VolInvol_EarlyLate AS (
SELECT Fixed_month,TenureChurners, Count(voluntario) AS VoluntaryChurners, count(involuntario) AS InvoluntaryChurners
FROM UnionChurners
WHERE TenureChurners is not null
GROUP BY Fixed_Month,TenureChurners
ORDER BY Fixed_Month
)
-----------------------------------------------RGU Churn---------------------------------------------------------

SELECT Fixed_month, COUNT (Fixed_account) AS Churners, COUNT (B_VO_id) AS RGU_VO, count(B_BB_id) AS RGU_BB, count(B_tv_nm) AS RGU_TV
FROM UnionChurners 
WHERE TenureChurners is not null
GROUP BY Fixed_Month
ORDER BY Fixed_month*/
