...
REALCHURNERS AS(
 ...
 FROM CHURNERSCRM c  LEFT JOIN FIRSTCHURN f ON c.ACT_ACCT_CD = f.ACT_ACCT_CD AND f.PrimerChurn <= c.MaxFecha
...
,Churners AS (
    SELECT f.*, safe_cast(churners AS string) As AllChurners
    FROM FixedCustomerBase_MvmtFlag f LEFT JOIN ChurnerMovementTable c ON Fixed_Account=act_acct_cd
    AND Fixed_month=safe_cast(MesChurnF as date)
)

SELECT DISTINCT Fixed_Month, AllChurners, Count(*)
FROM Churners
WHERE AllChurners IS NOT NULL
GROUP BY 1,2
ORDER BY FIXED_MONTH
