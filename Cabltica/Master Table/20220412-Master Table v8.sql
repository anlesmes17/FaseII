...
,MobileUsefulFields_BOM AS (
    SELECT Date_Trunc(FECHA_PARQUE, month) AS Month, Contrato, ID_CLIENTE, NUM_IDENT, ID_ABONADO, AGRUPACION_COMUNIDAD
    FROM `gcp-bia-tmps-vtr-dev-01.gcp_temp_cr_dev_01.20220411_cabletica_fmc_febrero` 
    WHERE DES_SEGMENTO_CLIENTE <>"Empresas - Empresas" AND DES_SEGMENTO_CLIENTE <>"Empresas - Pymes" AND (DES_USO ="MOVIL CONTRATO" OR DES_USO="DATOS MOVIL POSPAGO (UMTS/DATACARD)" OR DES_USO="DATOS MOVIL POSPAGO (UMTS/DATACARD)")
)
...
,TenureClosingBase AS (
    SELECT DISTINCT ID_ABONADO, MIN(MESES_ANTIGUEDAD) AS MESES_ANTIGUEDAD
...
,MobileChurners AS (
Select DISTINCT e.ID_ABONADO AS Account_EOM, b.ID_ABONADO AS ID_BOM, b.ID_ABONADO As Account_BOM
