...
,GrossAdds_Flag as(
  ...
  count(distinct Gross_Adds) as kpi_meas from FMC_Table group by 1,2,3,4,5,6,7,8,9
)
...
,ActiveBase_Flag1 as(
  ...
  count(distinct Active_Base) as kpi_meas from FMC_Table group by 1,2,3,4,5,6,7,8,9
)
...
,TechTickets_Flag as(
  ...
  round(Tech_Tix_per_100_Acct*100,2) as kpi_meas from S5_CX_KPIs
)
...
round(Customers_w_MRC_Changes*100,2) as kpi_meas From S3_CX_KPIs
)
...
,SalesSoftDx_Flag as(
  ...
  round(New_Sales_to_Soft_Dx*100,2) as kpi_meas From S3_CX_KPIs
)
...
,EarlyIssues_Flag as(
  ...
  round(New_Customer_Callers*100,2) as kpi_meas From S3_CX_KPIs
)
...
,LongInstall_Flag as(
  ...
  round(breech_cases_installs*100,2) as kpi_meas From S3_CX_KPIs
)
...
,EarlyTickets_Flag as(
  ...
  round(early_tech_tix*100,2) as kpi_meas From S3_CX_KPIs
)
...
,RepeatedCall_Flag as(
  ...
  round(Repeat_Callers*100,2) as kpi_meas From S5_CX_KPIs
)
...
,Join_DNA_KPIS as(
  ...
  From( select distinct Month,Opco,Market,MarketSize,Product,Biz_Unit,facet,journey_waypoint,kpi_name,kpi_meas From GrossAdds_Flag
...
,Join_Sprints_KPIs as(
  select distinct Month,Opco,Market,MarketSize,Product,Biz_Unit,facet,journey_waypoint,kpi_name,kpi_meas
  From( select distinct Month,Opco,Market,MarketSize,Product,Biz_Unit,facet,journey_waypoint,kpi_name,kpi_meas From Join_DNA_kpis
...
from( select Month,Opco,Market,MarketSize,Product,Biz_Unit,journey_waypoint,facet,kpi_name,kpi_meas from join_sprints_kpis
