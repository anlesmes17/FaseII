...
where Fixed_Month=cast('2022-11-01' as date) and activebom=1 and b_numrgus=0
)
,WithRGUs as(
select * From "db-analytics-dev"."dna_fixed_cr"
where (pd_vo_prod_nm IS NOT NULL OR pd_bb_prod_nm IS NOT NULL OR pd_tv_prod_nm IS NOT NULL) and date_trunc('Month',cast(dt as date))=date('2022-11-01')
)

Select distinct a.Fixed_Account,b.act_acct_cd
From NoRGUs a inner join withRGUs b on a.Fixed_Account=b.act_acct_cd
