...
WITH 
Fixed_Base AS(
  SELECT * FROM "lla_cco_int_san"."cwc_fix_stg_dashboardInput_sep_AL_new_mrc" 
)

,Mobile_Base AS(
  SELECT * FROM "cwc_mob_stg_dashboardinput_sep_avnf_mrcv2"
)
...
CASE WHEN b_numrgus is null THEN 0 else (cast(b_numrgus AS double)) end as b_numrgus_adj,
CASE WHEN e_numrgus is null THEN 0 else (cast(e_numrgus AS double)) end as e_numrgus_adj
...
,install_data as (
select 
    date_trunc('Month',cast (dt as date)) as Month_Table,
...
from (select * from "db-analytics-prod"."tbl_fixed_cwc")
...
/*
select distinct inst_month, sales_channel,count(distinct account_id)
from saleschannel_SO
group by 1,2
order by 1,2
*/
...
, FullTable_SalesChannel AS(
 ...
 else 'otro' END AS Sales_channel,
CASE when s.account_id is not null then 'Match'
else 'No match' end as saleschannel_flag
 FROM install_data f LEFT JOIN saleschannel_SO s
 ON f.act_acct_cd = cast(s.account_id as varchar) and f.install_month = s.inst_month
)

,S_Channel_MT as(
select distinct f.*,Install_Month, sales_channel
From Final_Flags f left join FullTable_SalesChannel
ON Month_Table=Month and act_acct_cd=Fixed_account
)




SELECT --distinct Month, sales_channel, count(distinct fixed_account)
distinct Month,sales_channel,sum((e_numrgus_adj - b_numrgus_adj))
FROM S_Channel_MT
where (e_numrgus_adj - b_numrgus_adj)>0
--where cast(install_Month as varchar)=('2022-05-01')
group by 1,2
order by 1,2
