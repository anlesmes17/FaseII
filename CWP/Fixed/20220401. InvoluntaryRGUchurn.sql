    ...
    ,NO_OVERDUE AS(
    SELECT DISTINCT DATE_TRUNC (dt, MONTH) AS MES, act_acct_cd AS Account, fi_outst_age, PD_VO_PROD_CD, PD_BB_PROD_CD, PD_TV_PROD_CD
    FROM UsefulFields t
    ...
    GROUP BY MES, account, fi_outst_age, PD_VO_PROD_CD, PD_BB_PROD_CD, PD_TV_PROD_CD
    )
    ,OVERDUELASTDAY AS(
    ...
    SELECT DISTINCT n.MES AS Month, n. account, l.ChurnTenureDays, PD_VO_PROD_CD, PD_BB_PROD_CD, PD_TV_PROD_CD
    FROM NO_OVERDUE n INNER JOIN OVERDUELASTDAY l ON n.account = l.account and n.MES = l.MES
    )
    ,InvoluntaryChurners AS(
    SELECT DISTINCT Month, SAFE_CAST(Account AS STRING) AS Account, ChurnTenureDays, PD_VO_PROD_CD, PD_BB_PROD_CD, PD_TV_PROD_CD
    ,CASE WHEN Account IS NOT NULL THEN "2. Fixed Involuntary Churner" END AS ChurnerType
    FROM INVOLUNTARYNETCHURNERS 
    GROUP BY Month, Account,ChurnerType, ChurnTenureDays, PD_VO_PROD_CD, PD_BB_PROD_CD, PD_TV_PROD_CD
   )
   SELECT COUNT(PD_VO_PROD_CD) AS RGU_VO, COUNT(PD_BB_PROD_CD) AS RGU_BB,COUNT(PD_TV_PROD_CD) AS RGU_TV,
