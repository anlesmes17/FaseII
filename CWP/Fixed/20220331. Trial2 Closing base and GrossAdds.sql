...
GrossAddsRGUs AS (
SELECT act_acct_cd,avgVO, avgBB, avgTV,month,
...
FROM GROSSADDSMONTH
GROUP BY act_acct_cd, vo,bb,tv,avgVO, avgBB, avgTV,month
)
SELECT month,count(act_acct_cd) AS GrossAdds, sum(VO) AS GrosAddsVO, sum(BB) AS GrossAddsBB, sum(TV) AS GrossAddsTV,
